...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    private static final String TABLE_ERROR_LOG = "ErrorLog";
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        db.execSQL("Create Table " + TABLE_ERROR_LOG + "(EDatetime TEXT, ErrorName TEXT)");
    }
    ...
    //Insert Error Log Report
    public Boolean InsertError(String Error_Name){
        Date currentTime = Calendar.getInstance().getTime();
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues CV = new ContentValues();
        CV.put("EDatetime", String.valueOf(currentTime));
        CV.put("ErrorName", Error_Name);

        long result = db.insert(TABLE_ERROR_LOG, null, CV);
        db.close();
        if(result == -1)
            return false;
        else
